    ...
    public sealed class EventManager
    {
        ...
        internal void AddAutowireInfos(Type[] types)
        {
            if (types != null)
            {
                ...
                {
                    EventCallAutowireAttribute attribute = types[i].GetCustomAttribute<EventCallAutowireAttribute>();
                    if (attribute != null)
                    {
                        if (types[i].GetInterface("IEventRegister") != null)
                        {
                            AutowireInfo autowire = new AutowireInfo(types[i], attribute);
                        ...
                        {
                            LogUtil.Warning($"类型{types[i].FullName}没有实现接口IEventRegister：EventCallAutowireAttribute特性只能注解在实现了接口IEventRegister的类上!");
                        }
#endif
                    }
                }
