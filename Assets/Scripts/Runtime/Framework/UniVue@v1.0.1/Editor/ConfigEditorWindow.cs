...
namespace UniVue.Editor
{
    internal sealed class ConfigEditorWindow : EditorWindow
    {
        ...
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("创建的VueConfig配置文件的名称");
        ...
        {
            VueConfig config = ConfigBuilderInEditor.CreateViewConfig<VueConfig>(_configFileName, _saveDirectory);
            if(config != null)
            {
                AssetDatabase.CreateAsset(config, $"{_saveDirectory}{_configFileName}.asset");
